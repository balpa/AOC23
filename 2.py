...
pinput = """Game 1: 10 green, 5 blue; 1 red, 9 green, 10 blue; 5 blue, 6 green, 2 red; 7 green, 9 blue, 1 red; 2 red, 10 blue, 10 green; 7 blue, 1 red
...
for stc in pinput.split('\n'):
    sep = stc.split(': ')
    revealed = re.split(r'[,;]\s*', sep[1])
    isValid = True

    for attempt in revealed:
        atp = attempt.split(' ')

        if rules[atp[1]] < int(atp[0]):
            isValid = False

    res.append(int(sep[0].split(' ')[1])) if isValid else None    
